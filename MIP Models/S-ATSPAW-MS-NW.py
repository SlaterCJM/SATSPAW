...
def ICmodelx(name,n,K,Tmax,M,pik,rik,tij,T,gh):
    ...
    y={ (i+1,j+1) : m.continuous_var(name='y_{0}_{1}'.format(i+1,j+1),lb=0) for i in range(n) for j in range(n)}# if (i!=j) & (j!=0)}
    ...
    for i in range(1,n):      
            m.add_constraint( m.sum(y[i+1,j+1] for j in range(n) if (j!=i)) + m.sum(rik[t,i,k]*z[i+1,k+1,t+1] for k in range(K) for t in range(T) ) <=Tmax  )
    ...
    for i in range(1,n):
         for j in range(n): 
                 if i != j:                
                        m.add_constraint(y[i+1,j+1] <=  (Tmax -np.min(rik[:,i,:])) *x[i+1,j+1])  
                        #m.add_constraint(y[i+1,j+1] >=  x[i+1,j+1])  #NO ES NECESARIA DADA LA MEJOR COTA DE ABAJO
          
                
    for j in range(1,n):
        
        m.add_constraint(m.sum(y[i+1,j+1] for i in range(n) if (j!=i)) +m.sum(x[i+1,j+1]*tij[i,j] for i in range(n) if (j!=i)) <= m.sum(y[j+1,p+1] for p in range(n) if (p!=j) ) )
    ...
    for i in range(1,n):
        for j in range(n): 
                ...
                if (i!=j) :
                    ...
                    m.add_constraint(y[i+1,j+1]  >=  m.sum(tij[k,i]*x[k+1,i+1] for k in range(n) if (k!=i)) + np.max(tij[:,i])*(x[i+1,j+1]-1)  )

    #m.export_as_lp(str(name)+'-MS.lp')
    m.print_information()
    a=time.time()
    #print(len(x))
    ...
    #gc.collect()
    '''
    return dx,dz,dy,sol,bestbound,mipgap,etime
...
def ALLRUNX(prob,aa,tt,nomb,gh):
    ...
    for i in  range(len(prob)):
        ...
        tij[tij==0]=1
        rik[rik==0]=1
        x,z,y,sol,bestbound,mip,etime=ICmodelx(prob[i],n,K,Tmax,M,pik,rik,tij,T,gh)
        xv.append(x) 
        zv.append(z)
        yv.append(y)
        #vv.append(v)
    ...
    #writer.save()     
    writer.close() 
    return #xv,zv,yv#,solu,bestb,mip,ttime 
    #'''
...
if __name__ == "__main__":
    ...
    ALLRUNX(['br17','ftv33','ftv35','ftv38','ftv44','ftv47','ft53','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],3,0.3,'S-ATSPAW-MS-SINW-COTANUEVA-06-06',3600)
