...
def ICmodelx(name,n,K,Tmax,M,pik,rik,tij,T,gh):
    # DEF MODELO Y VARIABLES
    # Alternativa utilizando  tipo_var_dict

    #arcos = [ ( i , j ) for i in range(N) for j in range(N)  ]
    ...
    #x={ (i+1,j+1) : m.continuous_var(name='x_{0}_{1}'.format(i+1,j+1),lb=0, ub=1) for i in range(n) for j in range(n) }
    ...
    #z={ (i+1,k+1,t+1) : m.continuous_var(name='z_{0}_{1}_{2}'.format(i+1,k+1,t+1),lb=0, ub=1) for i in range(n) for k in range(K) for t in range(T) }#if i!=0}
    #y={ (i+1,j+1) : m.continuous_var(name='y_{0}_{1}'.format(i+1,j+1)) for i in range(n) for j in range(n)}# if (i!=j) & (j!=0)}
    ...
    # DEF FUNC OBJETIVO # quizas colocar lowerbound de 1 en los y_ij
    #print(len(x))
    #print(x)
    ...
    for i in range(1,n):  # si se ocupa la última cota de los ui no es necesario hace range(n) se puede exluir el 1.
        for j in range(1,n):
            if i != j:
                m.add_constraint(u[i+1]+tij[i,j]*x[i+1,j+1]- (1-x[i+1,j+1])*M   <=u[j+1]  )
    ...
    for i in range(1,n):
            ...
            for k in range(K):
                    ...
                    for t in range(T):
                            ...
                            m.add_constraint(z[i+1,k+1,t+1]+ m.sum(z[f+1,a+1,t+1] for f in range(n) for a in range(K) if (i!=f) )/K <=1 )
    ...
    for i in range(1,n):
            ...
            for k in range(K):
                    ...
                    for t in range(T):
                            ...
                            m.add_constraint(z[i+1,k+1,t+1]+ m.sum(z[i+1,a+1,g+1] for a in range(K) for g in range(T) if (g!=t) )/K <=1 )
        ...
        for f in range(n):
            ...
            for k in range(K):
                for a in range(K):
                    for t in range(T):
                        if(f!=i)  : #(a!=k)
                            m.add_constraint(z[i+1,k+1,t+1]+z[f+1,a+1,t+1]  <=1 )
    ...
    #for i in range(1,n):
    #    for k in range(K):
    #        m.add_constraint(m.sum(z[i+1,k+1,t+1] for t in range(T) ) <=1  )
            
    #for i in range(n):
     #   for t in range(T):
      #      m.add_constraint(m.sum(z[i+1,k+1,t+1] for k in range(K) ) <=K*m.sum(x[j+1,i+1] for j in range(n) if (i!=j) ) )
            
    for i in range(1,n):
       
        m.add_constraint(m.sum(z[i+1,k+1,t+1] for k in range(K) for t in range(T) ) <=K*m.sum(x[j+1,i+1] for j in range(n) if (i!=j) ) )
    
    #m.add_constraint(u[4]==8)  
    #m.add_constraint(u[2]==13)  
    #for i in range(1,n):
       
    #    m.add_constraint(u[i+1] <= Tmax*m.sum(x[j+1,i+1] for j in range(n) if (i!=j) ))
        
    #m.add_constraint(x[1,6]==1)   
    #m.add_constraint(x[6,1]==1) 
    
    #'''
    for i in range(1,n):
       
        #m.add_constraint(m.sum(x[j+1,i+1] for j in range(n) if j!=i )- m.sum(z[i+1,k+1,t+1] for k in range(K) for t in range(T))/K >=0) 
    ...
    if  str(m.get_solve_status ( )) =='JobSolveStatus.INFEASIBLE_SOLUTION' or (str(m.get_solve_status ( )) == 'JobSolveStatus.UNKNOWN'):
        sol=0
        bestbound=0
        mipgap=100
        etime=b-a
        dx=0
        dz=0
        #dv=msolution.get_value_dict(v,keep_zeros=True, precision=1e-13)
        #dy=msolution.get_value_dict(y,keep_zeros=True, precision=1e-13)
        du=0
        #dw=0
        print('INFEASIBLE')
    else:
        ...
        #print(len(dx),len(msolution.get_value_dict(x,keep_zeros=True, precision=1e-13)),len(x))
        #print(dx)
        #print('...')
    ...
    #gc.collect()
    '''
    return dx,dz,du,sol,bestbound,mipgap,etime  #dv dy
...
if __name__ == "__main__":
    ...
    ALLRUNX(['br17','ftv33','ftv35','ftv38','ftv44','ftv47','ft53','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],3,0.3,'S-ATSPAW-MTZ-SINW-06-06',3600)
