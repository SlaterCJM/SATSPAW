...
def RUN_ILS_VNP_11_PK(prob,tt,aa,nomb,niter,M,alpha,a1,a2,b1,c0,kk):
    ...
    for nn in range(M):
        for i in  range(len(prob)):
                        ...
                        inprofit,profit,tiempo,gap,mejora=xILS_11_NP_XX(Tmax,tij,rik,pik,n,K,T,rng,niter,alpha,a1,a2,b1,c0,mipsol[str(prob[i])])
...
def xILS_11_NP_ACT( DTYPE_FLOAT Tmax, np.ndarray[DTYPE_FLOAT, ndim=2] tij, np.ndarray[DTYPE_FLOAT, ndim=3] r, 
    np.ndarray[DTYPE_FLOAT, ndim=2] p, DTYPE_INT n, DTYPE_INT k, DTYPE_INT cantidadt, DTYPE_INT rng,
    DTYPE_INT niter, DTYPE_FLOAT alpha, DTYPE_FLOAT a1, DTYPE_FLOAT a2, 
    DTYPE_FLOAT b1, DTYPE_FLOAT c0, DTYPE_FLOAT cota, DTYPE_INT p1, DTYPE_INT p2,DTYPE_INT p3,DTYPE_INT p4,DTYPE_INT ls1, DTYPE_INT ls2
):
...
def RUN_ILS_VNP_11_RES(prob,tt,aa,nomb,niter,M,alpha,a1,a2,b1,c0,p1,p2,p3,p4,ls1,ls2):
    ...
    for nn in range(M):
        for i in  range(len(prob)):
                        ...
                        #print('CANGTIDADT',T)
                        #print(nn,profits,profits[nn])
                        inprofit,profit,tiempo,gap,mejora=xILS_11_NP_ACT(Tmax,tij,rik,pik,n,K,T,rng,niter,alpha,a1,a2,b1,c0,mipsol[str(prob[i])],p1,p2,p3,p4,ls1,ls2)
                        print(profit)
                        print(f'{gap*100:.2f}')
                        print(f'{tiempo:.4f}')
    ...
    Res=np.c_[Nombre,ncities,inprofit,profit,Tempo]#,mejoras]
    writer=pd.ExcelWriter(str(nomb)+str('Activación: ')+str(p1)+str(p2)+str(p3)+str(p4)+str(ls1)+str(ls2)+'.xlsx')
    Res=pd.DataFrame(Res)#,columns=Columns)
    Res.to_excel(writer,'Results')
    writer.close()     
