...
def calcdisentrada_2(ruta,u, tij, i, j):
    cdef int k
    u[i+1] = u[i] + tij[ruta[i], ruta[i+1]]
    
    for k in range(i + 2, len(ruta)):
...
def xswap_seguido_en_ruta_infactible(ruta, tij, u):
   
        sel = np.random.choice(ruta[1:-2])
      
        index_sel = list(ruta).index(sel)
       
        ruta[index_sel], ruta[index_sel + 1] = ruta[index_sel + 1], ruta[index_sel]  
        
        calcdisentrada_swap(ruta,u, tij, index_sel)
...
def xtwoopt_v1_f_opt(ruta, u, tij):
    cdef int i,j,n
    ...
    while imp:
        ...
        for i in range(n - 2):
            for j in range(i + 2, n - 1):   #(i+1)
                ...
                #prev_segment = ruta[i + 1:j + 1]
                #ruta[i + 1:j + 1] = np.array(list(reversed(prev_segment)))
                
                #ul=u[-1].copy()
               
                #sm=np.sum(u)
                calcdisentrada_2(ruta,u, tij, i, j)
                ...
                if u[-1] < ul or  (u[-1] <= ul and sma < sm):
                    ...
                    #ruta[i + 1:j + 1] = prev_segment
                    u[:]=ut
                    ruta[i + 1 : j + 1] = ruta[i + 1 : j + 1][::-1]
                    
            if imp:
                break  # Salir del ciclo externo también
    return
...
def xILS_11_NP_XX(  DTYPE_FLOAT Tmax, np.ndarray[DTYPE_FLOAT, ndim=2] tij, np.ndarray[DTYPE_FLOAT, ndim=3] r, 
    ...
    DTYPE_FLOAT b1, DTYPE_FLOAT c0, DTYPE_FLOAT cota
):
    ...
    #u=np.array(u)
    tiempot =np.array([0 for i in range(cantidadt)])
    
    nodos=np.array([0 for i in range(cantidadt)])
...
def RUN_ILS_VNP_11(prob,tt,aa,nomb,niter,M,alpha,a1,a2,b1,c0):
    ...
    for nn in range(M):
        for i in  range(len(prob)):
                        ...
                        inprofit,profit,tiempo,gap,mejora=xILS_11_NP_XX(prob[i],Tmax,tij,rik,pik,n,K,T,rng,niter,alpha,a1,a2,b1,c0,mipsol[i])
                        print(profit)
                        print(f'{gap*100:.2f}')
                        print(f'{tiempo:.4f}')
...
def xLS_knapsack(
    ...
    for i in range(t):# (len(ruta)-2):#(t):
      ...
      if nodos[i]!=0:
          idx = np.where(ruta[1:-1] == nodos[i])[0][0]
          #print(idx,type(idx))
          profit,zsol=knap.knapsack(rik[i,nodos[i],:], pik[nodos[i],:]).solve(tmax-u[idx+1])
          #print(profit,type(profit), zsol,type(zsol))
...
def xILS_11_NP_XX( list prob, DTYPE_FLOAT Tmax, np.ndarray[DTYPE_FLOAT, ndim=2] tij, np.ndarray[DTYPE_FLOAT, ndim=3] r, 
    np.ndarray[DTYPE_FLOAT, ndim=2] p, DTYPE_INT n, DTYPE_INT k, DTYPE_INT cantidadt, DTYPE_INT rng,
    DTYPE_INT niter, DTYPE_FLOAT alpha, DTYPE_FLOAT a1, DTYPE_FLOAT a2, 
    DTYPE_FLOAT b1, DTYPE_FLOAT c0, DTYPE_FLOAT cota
):
    ...
    best_profit=xLS_knapsack(Tmax,p,tij,r,u,zz,cantidadt,n,nodos,sol)
    ...
    while i <= iterMax and tlim < 360:
        ...
        if rd0 <= b1 :
            ...
            if np.all(u <= Tmax):
                ...
                profit_sol=xLS_knapsack(Tmax,p,tij,r,u,zz,cantidadt,n,nodos,sol)
            ...
            sol,zz=xcheapest_ins_dist_mod_profit(Tmax,p,tij,r,u,zz,cantidadt,n,sol,nodos) 
            #sol,zz=xcheapest_ins_dist_mod_profit_v2(Tmax,p,tij,r,u,zz,cantidadt,n,sol,nodos) 
            xtwoopt_v1_f_opt(sol, u, tij)
            #xtwoopt_v1_f_full_ciclo(sol, u, tij,i)
        ...
        if len(np.where(nodos==0)[0]) >=1 or len(sol)> (cantidadt +2):
            ...
            if len (sol) > 2:
                ...
                profit_sol=0
            if np.any(u >= Tmax) or np.any(tiempot > Tmax):
...
def xILS_11_NP_XX2( list prob, DTYPE_FLOAT Tmax, np.ndarray[DTYPE_FLOAT, ndim=2] tij, np.ndarray[DTYPE_FLOAT, ndim=3] r, 
    np.ndarray[DTYPE_FLOAT, ndim=2] p, DTYPE_INT n, DTYPE_INT k, DTYPE_INT cantidadt, DTYPE_INT rng,
    DTYPE_INT niter, DTYPE_FLOAT alpha, DTYPE_FLOAT a1, DTYPE_FLOAT a2, 
    DTYPE_FLOAT b1, DTYPE_FLOAT c0, DTYPE_FLOAT cota
):
    ...
    while i <= iterMax and tlim < 360:
        ...
        if rd0 <= b1 :
            ...
            if (np.any(u[:-1] >= Tmax)) or (u[-1]> Tmax):
                  ...
                  it=1
            #if np.all(u <= Tmax):
            if (np.any(u[:-1] < Tmax)) and (u[-1]<= Tmax):
                #print('MEJORO INFAC U',i)
                if it==1 or las==1:
                    xls_assignment0(sol, cantidadt, r, nodos, zz)
                    nuasin=nuasin+1
...
def calcdisentrada_2(np.ndarray[DTYPE_INT, ndim=1] ruta, 
    ...
    cdef np.ndarray[DTYPE_FLOAT, ndim=1] tiempos
    #u[i+1] = u[i] + tij[ruta[i], ruta[i+1]]
    ...
    #tiempos=np.array([tij[ruta[j],ruta[j+1]] for j in range(i,len(ruta)-1)])
    tiempos = tij[ruta[i:len(ruta)-1], ruta[i+1:len(ruta)]]
    #print(tiempos)
    #suma = np.cumsum(tiempos) + u[i]
    
    u[i+1:]=np.cumsum(tiempos) + u[i]#suma
    #print(u)
    #print('**')
'''    
...
def calcdisentrada_2(np.ndarray[DTYPE_INT, ndim=1] ruta, 
                     ...
                     int i, int j):
    cdef int k
    cdef DTYPE_FLOAT suma = u[i]
    cdef int len_ruta= len(ruta)
    
    for k in range(i + 1, len_ruta):
        suma += tij[ruta[k - 1], ruta[k]]
        u[k] = suma
...
def xcheapest_ins_dist_mod_profit(
    ...
    # Calcular beneficios por nodo vectorizadamente
    ruta_act = ruta.tolist()
    profitnodo = np.sum(zz[:, ruta[1:-1], :] * pik[ruta[1:-1], :], axis=(0, 2))
    #print(len(ruta_act))
    # Encontrar nodo menos rentable
    indexnodo_elim = np.argmin(profitnodo)
    nodo_eliminado = ruta_act.pop(indexnodo_elim + 1)
    rback=ruta_act.copy()
    sm = np.sum(u)
    #nodo_eliminado = ruta[indexnodo_elim + 1]
    #print(len(ruta_act))
    ...
    while len(nv) > 0:
        ...
        minindex = np.argmin(dist)
        #ruta_mod = np.insert(ruta_act, minindex + 1, sel)
        ruta_act.insert(minindex + 1, sel)
        # Guardar estado actual
        ul = u[-1]
        #sm = np.sum(u)
        ...
        # Actualizar tiempos de entrada
        selindex = min(indexnodo_elim + 1, minindex + 1)
        calcdisentrada_swap(ruta_act, u, tij, selindex)
        
        sma = np.sum(u)
        
        # Verificar si la solución es mejor
        if u[-1] < ul or (u[-1] <= ul and sma < sm):
            ruta = np.array(ruta_act)
            zc[:, sel, :] = temp
...
def RUN_ILS_VNP_11(prob,tt,aa,nomb,niter,M,alpha,a1,a2,b1,c0):
    ...
    for nn in range(M):
        #for i in  range(len(prob)):
                        ...
                        #print('CANGTIDADT',T)
                        #print(nn,profits,profits[nn])
                        inprofit,profit,tiempo,gap,mejora=xILS_11_NP_XX(Tmax,tij,rik,pik,n,K,T,rng,niter,alpha,a1,a2,b1,c0,mipsol[str(prob)])
                        print(profit)
                        print(f'{gap*100:.2f}')
                        print(f'{tiempo:.4f}')
...
def xILS_11_NP_XX_ACT( DTYPE_FLOAT Tmax, np.ndarray[DTYPE_FLOAT, ndim=2] tij, np.ndarray[DTYPE_FLOAT, ndim=3] r, 
    np.ndarray[DTYPE_FLOAT, ndim=2] p, DTYPE_INT n, DTYPE_INT k, DTYPE_INT cantidadt, DTYPE_INT rng,
    DTYPE_INT niter, DTYPE_FLOAT alpha, DTYPE_FLOAT a1, DTYPE_FLOAT a2, 
    DTYPE_FLOAT b1, DTYPE_FLOAT c0, DTYPE_FLOAT cota, DTYPE_INT p1, DTYPE_INT p2,DTYPE_INT p3,DTYPE_INT p4,DTYPE_INT ls1, DTYPE_INT ls2,DTYPE_INT asig, DTYPE_INT knap,DTYPE_INT timelimit ):
    ...
    while  tlim < timelimit:  #i <= iterMax or 
        ...
        elif rd <= a1 + a2 and p2 == 1:
             
             
             xswap_aleatorio_en_ruta_infactible(sol, tij, u)
             it=0
        elif rd <= 1 and p3==1:#0.6:
        ...
        rd1=np.random.random()
        if len(sol) < (cantidadt+2) and rd1 <= c0 and p4 == 1:
            sol,u=anadir_nodo_infactible(sol,tij,n,nodos)
            it=1
            #u=np.array(u) 
            ksum=ksum+1
...
def RUN_ILS_VNP_11_RES(prob,tt,aa,nomb,niter,M,alpha,a1,a2,b1,c0,p1,p2,p3,p4,ls1,ls2,asig,knap,timelimit):
    ...
    Res=np.c_[Nombre,ncities,inprofit,profit,Tempo]#,mejoras]
    #writer=pd.ExcelWriter(str(nomb)+str('Activación: ')+str(p1)+str(p2)+str(p3)+str(p4)+str(ls1)+str(ls2)+str(asig)+str(knap)+'.xlsx')
    writer=pd.ExcelWriter(str(nomb)+'.xlsx')
    Res=pd.DataFrame(Res)#,columns=Columns)
    Res.to_excel(writer,'Results')
    writer.close()
...
def RUN_ILS_VNP_11_RES_K20(prob,tt,aa,kk,nomb,niter,M,alpha,a1,a2,b1,c0,p1,p2,p3,p4,ls1,ls2,asig,knap):
    ...
    for nn in range(M):
        for i in  range(len(prob)):
                        ...
                        #pik = np.loadtxt('instancias/pik'+str(prob[i])+'-'+str(tt)+'-'+str(kk)+'.txt') # beneficio con recurso
                        #rik = np.loadtxt('instancias/rikt'+str(prob[i])+'-'+str(tt)+'-'+str(kk)+'.txt') # tiempo que toma realizar actividad
                        #tij = np.loadtxt('instancias/tij'+str(prob[i])+'.txt') # matriz de tiempos
                        #Tmax= np.loadtxt('instancias/tmax'+str(prob[i])+'-'+str(aa)+'.txt') # matriz de tiempos
                        
                        pik = np.loadtxt('pik'+str(prob[i])+'-'+str(tt)+'-'+str(kk)+'.txt') # beneficio con recurso
                        rik = np.loadtxt('rikt'+str(prob[i])+'-'+str(tt)+'-'+str(kk)+'.txt') # tiempo que toma realizar actividad
                        tij = np.loadtxt('tij'+str(prob[i])+'.txt') # matriz de tiempos
                        ...
                        #print('CANGTIDADT',T)
                        #print(nn,profits,profits[nn])
                        inprofit,profit,tiempo,gap,mejora=xILS_11_NP_XX_ACT(Tmax,tij,rik,pik,n,K,T,rng,niter,alpha,a1,a2,b1,c0,mipsol[str(prob[i])],p1,p2,p3,p4,ls1,ls2,asig,knap)
                        print(profit)
                        print(f'{gap*100:.2f}')
                        print(f'{tiempo:.4f}')
    ...
    Res=np.c_[Nombre,ncities,inprofit,profit,Tempo]#,mejoras]
    #writer=pd.ExcelWriter(str(nomb)+str('Activación: ')+str(p1)+str(p2)+str(p3)+str(p4)+str(ls1)+str(ls2)+str(asig)+str(knap)+'.xlsx')
    writer=pd.ExcelWriter(str(nomb)+'K=20.xlsx')
    Res=pd.DataFrame(Res)#,columns=Columns)
    Res.to_excel(writer,'Results')
    writer.close()
...
#	RUN_ILS_VNP_11_RES(['br17','ft53','ftv33','ftv35','ftv38','ftv44','ftv47','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],0.3,3,'OCTUBRE-SINASIGNACION',10000,10,0.013,0.23,0.011,0.271,0.156,1,1,1,1,1,1,0,1)
#       RUN_ILS_VNP_11_RES(['br17','ft53','ftv33','ftv35','ftv38','ftv44','ftv47','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],0.3,3,'OCTUBRE-SINKNAP',10000,10,0.013,0.23,0.011,0.271,0.156,1,1,1,1,1,1,1,0)
#       RUN_ILS_VNP_11_RES(['br17','ft53','ftv33','ftv35','ftv38','ftv44','ftv47','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],0.3,3,'OCTUBRE-SINPERTURBACIONFUERADERUTA',10000,10,0.013,0.23,0.011,0.271,0.156,1,1,0,1,1,1,1,1)
#       RUN_ILS_VNP_11_RES(['br17','ft53','ftv33','ftv35','ftv38','ftv44','ftv47','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],0.3,3,'OCTUBRE-SINCHEAPEST',10000,10,0.013,0.23,0.011,0.271,0.156,1,1,1,1,1,0,1,1)
#	RUN_ILS_VNP_11_RES(['br17','ft53','ftv33','ftv35','ftv38','ftv44','ftv47','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],0.3,3,'OCTUBRE-PERTEQUIPROBABLE',10000,10,0.013,1/3,1/3,0.271,0.156,1,1,1,1,1,1,1,1)
#	RUN_ILS_VNP_11_RES(['br17','ft53','ftv33','ftv35','ftv38','ftv44','ftv47','ftv55','ftv64','ftv70','ftv90','kro124p','ftv100','ftv110','ftv120','ftv130','ftv140','ftv150','ftv160','ftv170'],0.3,3,'OCTUBRE-LSEQUIPROBABLE',10000,10,0.013,0.23,0.011,0.5,0.156,1,1,1,1,1,1,1,1)
